...
function tim($date){
	...
	$str = '';
	if($days > 0) $str .= tim_name($days,array('день','дня','дней')).' ';
	if($hours > 0) $str .= tim_name($hours,array('час','часа','часов')).' ';
	if($minutes > 0) $str .= tim_name($minutes,array('минута','минуты','минут')).' ';
	if($seconds > 0) $str .= tim_name($seconds,array('секунда','секунды','секунд'));
